...
public class IpPreFilter extends ZuulFilter {
    ...
    private Logger logger = LoggerFactory.getLogger(IpPreFilter.class);
    ...
    public Object run() {
        ...
        logger.info("IpPreFilter--------------------------"+ System.currentTimeMillis());

        // 在黑名单中禁用
        if (StringUtils.isNotBlank(ip) && ip.equals("47.91.232.186")) {
            ctx.set("isSuccess", false);
            ctx.setSendZuulResponse(false);//请求不再向后转发
            HashMap<String, Object> result = new HashMap<>();
            result.put("code", 403);
            ctx.setResponseBody(JsonUtils.toJson(result));
